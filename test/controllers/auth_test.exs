defmodule Shittalker.AuthTest do
  use Shittalker.ConnCase
  alias Shittalker.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(Shittalker.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Shittalker.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Shittalker.User{id: 123})
      |> send_resp(:ok, "")
